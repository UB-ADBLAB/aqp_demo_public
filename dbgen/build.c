...
#define PART_SUPP_BRIDGE(tgt, p, s) \
    ...
    tgt = (p + s *  (tot_scnt / SUPP_PER_ITEM +  \
...
/*
 * TODO: use use nation and region code for interesting queries
 * no zip for i will put that in later
 * 
 */


long
mk_cust(DSS_HUGE n_cust, customer_t * c, DSS_HUGE ware_id, DSS_HUGE dist_id)
{
	DSS_HUGE        i;
	static int      bInit = 0;
	...
	c->nation_code = i;
	c->district_key = dist_id;
	c->warehouse_key = ware_id;
	gen_phone(i, c->phone, (long) C_PHNE_SD);
	pick_str(&c_mseg_set, C_MSEG_SD, c->mktsegment);
	RANDOM(temp, 0, 99, OFFSET_SD);
	if (temp < 10)
		strcpy(c->credit,  C_BC);
	else
		strcpy(c->credit, C_GC);
	c->creditlim = C_CRDT_LIM;
    RANDOM(i, C_DISC_MIN, C_DISC_MAX, C_DISC_SD);
	c->discount = (float) i/ 10000;
	c->balance = C_BANLANCE*100;
	c->ytdpayment = C_YTD_PAYMENT;
	c->pymentcnt = C_PAYMENT_CNT;
	c->deliverycnt = C_DELIVERY_CNT;
	TEXT(C_CMNT_LEN, C_CMNT_SD, c->data);
...
long
mk_district(DSS_HUGE n_dist, district_t * d, DSS_HUGE warehosuekey, DSS_HUGE nextoid)
{
	DSS_HUGE	i;
	static int bInit = 0; 
	static char szFormat[100];
	...
	if(!bInit){
		sprintf(szFormat, D_NAME_FMT, 9, HUGE_FORMAT + 1);
	}
    //
	d->distkey = n_dist;
	d->dist_ware_key = warehosuekey;
	sprintf(d->name, szFormat, D_NAME_TAG, n_dist);
	V_STR(D_ADDR_LEN, C_ADDR_SD, d->address);
	d->alen = (int)strlen(d->address);
	RANDOM(i, 0, (nations.count-1), D_NTRG_SD);
	d->nation_code = i;
	RANDOM(i, D_TAX_MIN, D_TAX_MAX, L_TAX_SD);
	d->tax =(float) i/ 10000;
	d->ytd = D_YTD;
	d->next_oid =  nextoid ;
	return (0);
...
long 
mk_ware(DSS_HUGE n_ware, ware_t *w)
{
	DSS_HUGE	i;
	static int bInit = 0;
	static char szFormat[100];
	...
	if(!bInit){
		sprintf(szFormat, W_NAME_FMT, 0, HUGE_FORMAT +1);
	}
	w->warehousekey = n_ware;
	sprintf(w->name, szFormat, W_NAME_TAG, n_ware);
	V_STR(W_ADDR_LEN, W_ADDR_SD, w->address);
	w->alen = (int)strlen(w->address);

	w->YTD = W_YTD;
	RANDOM(i, W_TAX_MIN, W_TAX_MAX, L_TAX_SD);
	w->tax = (float) i/10000;
	return (0);
	}
...
mk_order(DSS_HUGE index, order_t * o, long upd_num)
{
	...
	for (lcnt = 0; lcnt < o->lines; lcnt++)
	{
		...
		o->l[lcnt].ol_d_id = o->distkey;
		o->l[lcnt].ol_w_id = o->warekey;
		//ask here
		...
		if (scale >= 30000)
			RANDOM64(o->l[lcnt].itemkey, L_PKEY_MIN, L_PKEY_MAX, L_PKEY_SD);
		else
			RANDOM(o->l[lcnt].itemkey, L_PKEY_MIN, L_PKEY_MAX, L_PKEY_SD);
		rprice = rpb_routine(o->l[lcnt].itemkey);
		RANDOM(supp_num, 0, 3, L_SKEY_SD);
		PART_SUPP_BRIDGE(o->l[lcnt].suppkey, o->l[lcnt].itemkey, supp_num);
...
long
mk_order(DSS_HUGE index, order_t *order, DSS_HUGE dist_id, 
			DSS_HUGE ware_id, DSS_HUGE cust_id, int numberof_days)
{
	DSS_HUGE        clk_num;
	DSS_HUGE        tmp_date;
	static char     szFormat[100];
	...
	order->okey = index;
	/*
		TODO: need to check id the custkey order is matter
		spec says need a sequencital permutation for o_c_id
		but if the o_c_id's order does not matter( rand or not rand does not matter)
		let's go with  cust_id is from 1-3000 without sequential permutaiton 
	*/

	order->custkey = cust_id;
	order->warekey = ware_id;
	order->distkey =  dist_id;
	// RANDOM(tmp_date, O_ODATE_MIN, O_ODATE_MAX, O_ODATE_SD);
	// tmp_date = STARTDATE;
	strcpy(order->o_en_date, asc_date[TOTDATE - numberof_days]);
	if( numberof_days > 1)
	{
		RANDOM(order->carrierId, 1, 10, O_CKEY_SD);
	}
	/*
		try to see if the value is null works for print 
	*/
	/* else 
		order->carrierId = NULL; 
	*/
	RANDOM(order->o_ol_cnt, O_LCNT_MIN,O_LCNT_MAX,O_LCNT_SD);
	order->o_all_local = O_ALL_LOCAL;
	pick_str(&o_priority_set, O_PRIO_SD, order->o_order_priority);
	order->spriority = 0;
	return(0);
}

long 
mk_orderline(DSS_HUGE index, order_line_t *orderline, 
			  ...
			  DSS_HUGE dist_id, int numberof_days, DSS_HUGE temp_day)
{
	...
	orderline->ol_o_id =  order_id;
	orderline->ol_w_id = ware_id;
	orderline->ol_d_id = dist_id;
	orderline->ol_number = index;
	RANDOM(orderline->ol_item_id, 1, IT_NUMBER, IT_NUMBER_SD);  
	orderline->sup_w_key = ware_id;
	orderline->quantity = OL_QUANTITY;
	if((TOTDATE - numberof_days) + temp_day < TOTDATE -1)
	{
		strcpy(orderline->deliver_date, asc_date[(TOTDATE - numberof_days) + temp_day]);
	}
	else 
	{
		strcpy(orderline -> deliver_date, "\\N");		
	}
	tmp_price =  rpb_routine(orderline->ol_item_id);
	orderline->amount =  tmp_price * orderline->quantity;
	V_STR(OL_DIST_LEN, OL_DIST_SD, orderline->ol_dist_info);
	return (0);
}

long
mk_neworder(DSS_HUGE index, neworder_t *new_order,
			  DSS_HUGE ware_id, DSS_HUGE dist_id)
{
		new_order->no_orderId = index;
		new_order->no_districtId = dist_id;
		new_order->no_warehouseId = ware_id;

		return (0);
}

long
mk_part(DSS_HUGE index, item_t * p)
{
	DSS_HUGE        temp;
	long            snum;
	DSS_HUGE        brnd;
	...
	p->itemkey = index;
	RANDOM(p->i_im_id, P_IM_MIN, P_IM_MAX, P_IM_SD);
	...
	a_rnd(26, P_DATA_MAX,  P_CMNT_SD, p->data);
	RANDOM(temp, 0, 99, OFFSET_SD);
	if (temp < 10)
	{
		data_len = strlen(p->data);
		RANDOM(offset, 0, data_len-8, OFFSET_SD);
		memcpy(p->data+offset, "ORIGINAL",  8);
	}
	...
	for (snum = 0; snum < SUPP_PER_ITEM; snum++)
	{
		p->s[snum].itemkey = p->itemkey;
...
long
mk_stock(DSS_HUGE index, stock_t * st, DSS_HUGE warehousekey)
{
	DSS_HUGE i; 
	DSS_HUGE temp;
	int 			data_len;
	...
	if(!bInit)//does nothing but if needed later
	{
		bInit = 1;
	}
	st->stockkey = index;
	st->stockWarehousekey = warehousekey;
	RANDOM(i, 10, ST_QTY_MAX, ST_QTY_SD);
	st->quantity = i;
	st->ytd = 0;
	st->stock_order_cnt = 0;
	st -> stock_remote_cnt = 0;
	a_rnd(26, 50, ST_DATA_SD,st->s_data);
	RANDOM(temp, 0, 99, OFFSET_SD);
	if (temp < 10)
	{
		data_len = strlen(st->s_data);
		RANDOM(offset, 0, data_len-8, OFFSET_SD);
		memcpy(st->s_data+offset, "ORIGINAL",  8);
	}
...
long
mk_hist(DSS_HUGE index, hist_t* h, DSS_HUGE cust_id, 
		DSS_HUGE ware_id, DSS_HUGE dist_id){
	...
	//DSS_HUGE tmp_date;
	h->histCustID = index;
	h->histCustDistID = dist_id;
	h->histCustWareID = ware_id;
	h->histDistID = dist_id;
	h->histWareID = ware_id;
	// RANDOM(tmp_date, O_ODATE_MIN, O_ODATE_MAX, O_ODATE_SD);
	tmp_date = STARTDATE;
	strcpy(h->hist_date, asc_date[tmp_date - STARTDATE]);
	h->histAmt = H_AMOUT;
	V_STR(H_DATA_LEN,  H_DATA_SD, h->histData);
	return (0);
