 ...
 dbg_print(int format, FILE *target, void *data, int len, int sep)
{
	...
	case DT_MONEY:
		...
		dollars = cents / 100;
		cents %= 100;
		fprintf(target, "%ld.%02ld", (long)dollars, (long)cents);
		break;
	case DT_CHR:
...
static FILE *fp = NULL;
   ...
   PR_HUGE(fp, &c->district_key);
   PR_HUGE(fp, &c->warehouse_key);
   if (scale <= 3000)
   ...
   PR_MONEY(fp, &c->balance);
   PR_STR(fp, c->mktsegment, C_MSEG_LEN);
   PR_STR(fp, c->credit, C_CRDT_LEN);
   PR_MONEY(fp, &c->creditlim);
   PR_FLOAT(fp, &c->discount);
   PR_MONEY(fp, &c->ytdpayment);
   PR_MONEY(fp, &c->pymentcnt);
   PR_HUGE(fp, &c->deliverycnt);
   PR_VSTR(fp, &c->data, C_CMNT_LEN+1);
...
int pr_dist(district_t *d, int mode)
{
	...
	if (fp == NULL)
		fp = print_prep(DISTRCT, 0);

	PR_STRT(fp);
	PR_HUGE(fp, &d->distkey);			//distkey
	PR_HUGE(fp, &d->dist_ware_key); 	//disWarehousekey
	PR_VSTR(fp, &d->name, D_NAME_LEN); 	//name
	PR_VSTR(fp, &d->address, d->alen);	//address
	PR_HUGE(fp, &d->nation_code);		//nation code
	PR_FLOAT(fp, &d->tax);				//tax
	PR_MONEY(fp, &d->ytd);				//YTD
	PR_HUGE_LAST(fp, &d->next_oid);
    PR_END(fp);
...
pr_order(order_t *o, int mode)
{
    ...
    PR_STRT(fp_o);
    PR_HUGE(fp_o, &o->okey);
    PR_HUGE(fp_o, &o->distkey);
    PR_HUGE(fp_o, &o->warekey);
    PR_HUGE(fp_o, &o->custkey);
    PR_HUGE(fp_o, &o->carrierId);
    PR_STR(fp_o, o->o_en_date, DATE_LEN);
    PR_HUGE(fp_o, &o->o_ol_cnt);
    PR_HUGE(fp_o, &o->o_all_local);
    PR_STR(fp_o, o->o_order_priority, O_OPRIO_LEN);
    PR_HUGE_LAST(fp_o, &o->spriority);
    PR_END(fp_o);
 ...
 */
int
pr_orderline(order_line_t *orderline, int mode)
{
    ...
    if (fp_l == NULL || mode != last_mode)
        ...
        if (fp_l) 
            fclose(fp_l);
        fp_l = print_prep(ORDER_LINE, mode);
        last_mode = mode;
        }

    PR_STRT(fp_l);
    PR_HUGE(fp_l, &orderline->ol_o_id);
    PR_HUGE(fp_l, &orderline->ol_d_id);
    PR_HUGE(fp_l, &orderline->ol_w_id);
    PR_HUGE(fp_l, &orderline->ol_number);
    PR_HUGE(fp_l, &orderline->ol_item_id);
    PR_HUGE(fp_l, &orderline->sup_w_key);
    PR_STR (fp_l, orderline->deliver_date, DATE_LEN);
    PR_INT  (fp_l, orderline->quantity);
    PR_MONEY (fp_l, &orderline->amount);
    PR_VSTR_LAST(fp_l, orderline->ol_dist_info,OL_DIST_LEN);
    PR_END(fp_l);
 ...
 */
int
pr_stock(stock_t *s, int mode){
	static FILE *fp = NULL;
	static int last_mode = 0;
	long i;

	if(fp == NULL)
        fp = print_prep(STOCK, 0);
	PR_STRT(fp);
	PR_HUGE(fp, &s->stockkey);
	PR_HUGE(fp, &s->stockWarehousekey);
	PR_HUGE(fp, &s->quantity);
    PR_HUGE(fp, &s->ytd);
    PR_HUGE(fp, &s->stock_order_cnt);
    PR_HUGE(fp, &s->stock_remote_cnt);
	PR_VSTR_LAST(fp, s->s_data, P_DATA_LEN);
...
/*
 * print the given part/item
 */
int
pr_part(item_t *part, int mode)
{
static FILE *p_fp = NULL;

    if (p_fp == NULL)
        p_fp = print_prep(ITEM, 0);

   PR_STRT(p_fp);
   PR_HUGE(p_fp, &part->itemkey);
   ...
   PR_VSTR_LAST (p_fp, part->data, strlen(part->data));
   PR_END(p_fp);
...
pr_psupp(item_t *part, int mode)
{
   ...
   for (i = 0; i < SUPP_PER_ITEM; i++)
      {
      PR_STRT(ps_fp);
      PR_HUGE(ps_fp, &part->s[i].itemkey);
 ...
 */
/* will not use this for tpcc transaction*/
int
pr_part_psupp(item_t *part, int mode)
{
    tdefs[ITEM].name = tdefs[PSUPP].name;
    pr_part(part, mode);
    pr_psupp(part, mode);
...
int pr_hist(hist_t *h, int mode)
{
	static FILE *fp = NULL;
	if(fp == NULL)
		fp= print_prep(HIST, mode);
	PR_STRT(fp);
	PR_HUGE(fp, &h->histCustID);
	PR_HUGE(fp, &h->histCustDistID);
	PR_HUGE(fp, &h->histCustWareID);
	PR_HUGE(fp, &h->histDistID);
	PR_HUGE(fp, &h->histWareID);
    PR_STR(fp, h->hist_date, DATE_LEN);
    PR_MONEY(fp, &h->histAmt);
    PR_VSTR_LAST(fp, h->histData, H_DATA_LEN);
	PR_END(fp);
	return(0);
}
...
int pr_ware(ware_t *w, int mode)
{
	static FILE *fp = NULL;
	if (fp == NULL)
		fp = print_prep(WAREHOUSE, mode);

	PR_STRT(fp);
	PR_HUGE(fp, &w->warehousekey);
    PR_VSTR(fp, w->name, W_NAME_LEN);
	PR_STR(fp, w->address, w->alen);
	PR_FLOAT(fp, &w->tax);
	PR_MONEY_LAST(fp, &w->YTD);
	PR_END(fp);
	return(0);
}

int pr_neworder(neworder_t *neworder, int mode)
{
    static FILE *fp = NULL;
	if (fp == NULL)
		fp = print_prep(NEWORDER, mode);
	PR_STRT(fp);
	PR_HUGE(fp, &neworder->no_orderId);
    PR_HUGE(fp, &neworder->no_districtId);
    PR_HUGE_LAST(fp, &neworder->no_warehouseId);
	PR_END(fp);
	return(0);
}
