 ...
 */
void
hash_seq_init_extended(HASH_SEQ_STATUS *status, HTAB *hashp, bool allow_delete)
{
	status->hashp = hashp;
	status->curBucket = 0;
	status->allow_delete = allow_delete;
	status->curEntry = NULL;
	status->prevEntryPtr = NULL;
	if (!hashp->frozen)
		register_seq_scan(hashp);
}

/*
 * If status->allow_delete == true; there is a previously returned entry and
 * delete_prev is true, remove that entry. Otherwise, we ignore delete_prev.
 *
 */
void *
hash_seq_search_extended(HASH_SEQ_STATUS *status, bool delete_prev)
{
	HTAB	   *hashp;
	HASHHDR    *hctl;
	uint32		max_bucket;
	...
	if (status->allow_delete && delete_prev && status->prevEntryPtr != NULL)
	{
		HASHELEMENT *prevEntry;
		int freelist_idx;

		hashp = status->hashp;
		hctl = hashp->hctl;

		prevEntry = *status->prevEntryPtr;
		freelist_idx = FREELIST_IDX(hctl, prevEntry->hashvalue);
		...
		/* prevEntry->link may not be same as curEntry */
		*status->prevEntryPtr = prevEntry->link;

		prevEntry->link = hctl->freeList[freelist_idx].freeList;
		hctl->freeList[freelist_idx].freeList = prevEntry;
...
do_scan:
	if (status->allow_delete)
	{
		if (segment_ndx == -1)
		{
			/* we are continuing in the same bucket as prev */
			status->prevEntryPtr = &curElem->link;
		}
		else
		{
			/* we are at the first item in this non-empty bucket */
			Assert(curElem == segp[segment_ndx]);
			status->prevEntryPtr = &segp[segment_ndx];
		}
	}
