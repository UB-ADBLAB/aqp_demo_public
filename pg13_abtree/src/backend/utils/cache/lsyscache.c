 ...
 * get_cast_fn_oid
 *
 * Given two source and target type OIDs, look up the castfunc OID.
 * *p_castmethod is set if p_castmethod is not NULL upon return.
 *
 * If no such cast exists, *p_castmethod is set to '\0' (if p_castmethod is not
 * NULL) and the function returns InvalidOid (0). Unfortuately, in case that 0
 * is returned, it is not possible to determine whether it's because the cast
 * exists and the cast method is 'b'/'i', or the cast does not exist, if
 * p_castmethod is NULL. Hence, it is always necessary to pass an non-null
 * p_castmethod unless only the cast method 'f' is acceptable to the caller.
 *
 */
Oid
get_cast_fn_oid(Oid sourcetypeid, Oid targettypeid, char *p_castmethod)
{
	HeapTuple		ht;
	Form_pg_cast	cast;
	Oid				cast_fn;

	ht = SearchSysCache2(CASTSOURCETARGET,
						 ObjectIdGetDatum(sourcetypeid),
						 ObjectIdGetDatum(targettypeid));
	...
	cast = (Form_pg_cast) GETSTRUCT(ht);
	cast_fn = cast->castfunc;
	if (p_castmethod)
		*p_castmethod = cast->castmethod;
