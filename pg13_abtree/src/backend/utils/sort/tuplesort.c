 ...
 * it from caller-supplied values. An extra space of size ``extra'' is
 * allocated with the tuple buffer. The alignment can be any valid value
 * for pg_type.typalign or 'm' (for maximum alignment).
 */
void
tuplesort_putindextuplevalues_with_extra(Tuplesortstate *state, Relation rel,
										 ItemPointer self, Datum *values,
										 bool *isnull, Size extra,
										 char alignment)
{
	...
	stup.tuple = index_form_tuple_with_extra(RelationGetDescr(rel),
											 values, isnull, extra, alignment);
