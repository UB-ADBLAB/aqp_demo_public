					...
					GenericCosts *costs)
{
	 ...
	 *
	 * ABTree sample scan will provide the numIndexPages. If so, we can
	 * skip computing the number of index pages to fetch.
	 */
	numIndexPages = costs->numIndexPages;
	if (numIndexPages <= 0.0)
	{
...
void
abtcostestimate(PlannerInfo *root, IndexPath *path, double loop_count,
			   Cost *indexStartupCost, Cost *indexTotalCost,
			   Selectivity *indexSelectivity, double *indexCorrelation,
			   double *indexPages)
{
