			 ...
			 Oid useroid, char *out_dbname, bool override_allow_connections)
{
    ...
    /* 
     * Set up the abtree version store if needed before we return from
     * InitPostgres(). We can do that as early as we have lwlocks, shared
     * memory, and backend ID set up.
     */
    if (!bootstrap)
    {
		EpochSetup();
