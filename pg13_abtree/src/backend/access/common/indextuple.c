 ...
 *
 *		An extra space of size specified by the argument ``extra'' is
 *		allocated together with the tuple buffer. This is currently
 *		used by aggregate B-tree to reserve a space to fill the aggregated
 *		value in. The alignment can be any of the valid values for
 *		pg_type.typalign or 'm' (maximum alignment). The size of the tuple
 *		less extra is guaranteed to satisfy the alignment as long as extra
 *		is divisible by the alignment size.
 *
 * ----------------
 */
IndexTuple
index_form_tuple_with_extra(TupleDesc tupleDescriptor,
							Datum *values, bool *isnull,
							Size extra, char alignment)
{
	...
	/* 
	 * MAXALIGN'd size less extra should still be properly aligned to the
	 * specified alignment, assuming they are all power of 2's, which is
	 * also assumed to be true in include/c.h. We add this assertion here
	 * to make sure that is true.
	 */
	Assert((alignment == 'm') ? (MAXALIGN(size - extra) == (size - extra)) :
		(att_align_nominal(size - extra, alignment) == (size - extra)));
