 ...
 * To add an option:
 *
 * (i) decide on a type (integer, real, bool, string), name, default
	...
	{
		{
			...
			RELOPT_KIND_BTREE | RELOPT_KIND_ABTREE,
...
static relopt_typ typRelOpts[] =
{
	{
		{
			"aggregation_type",
			"The type of aggregations stored in the pivot nodes of the tree.",
			RELOPT_KIND_ABTREE,
			AccessExclusiveLock
		},
		INT8OID,
...
static relopt_proc procRelOpts[] =
{
	{
		{
			"agg_support",
			"The function that returns the ABTAggSupport struct.",
			RELOPT_KIND_ABTREE,
			AccessExclusiveLock
		},
		F_ABT_COUNT_SUPPORT, /* abt_count_support */
		false,
		1,
		{OIDOID},
		NULL
...
static bool parse_typ(const char *value, Oid *typeOid, int32 *typemod);
...
initialize_reloptions(void)
{
	...
	for (i = 0; typRelOpts[i].gen.name; ++i)
	{
		Assert(DoLockModesConflict(typRelOpts[i].gen.lockmode,
								   typRelOpts[i].gen.lockmode));
		++j;
	}
	for (i = 0; procRelOpts[i].gen.name; ++i)
	{
		Assert(DoLockModesConflict(procRelOpts[i].gen.lockmode,
								   procRelOpts[i].gen.lockmode));
		++j;
	}
	...
	for (i = 0; typRelOpts[i].gen.name; ++i)
	{
		relOpts[j] = &typRelOpts[i].gen;
		relOpts[j]->type = RELOPT_TYPE_TYP;
		relOpts[j]->namelen = strlen(relOpts[j]->name);
		++j;
	}
	for (i = 0; procRelOpts[i].gen.name; ++i)
	{
		if (!procRelOpts[i].delayed_parsing) {
			relOpts[j] = &procRelOpts[i].gen;
			relOpts[j]->type = RELOPT_TYPE_PROC;
			relOpts[j]->namelen = strlen(relOpts[j]->name);
			++j;
		}
		/* those proc relopt with delayed_parsing = true should be put
		 * at the end of the relOpts array */
	}

	for (i = 0; i < num_custom_options; i++)
	{
		if (custom_options[i]->type == RELOPT_TYPE_PROC &&
			((relopt_proc *) custom_options[i])->delayed_parsing) {
			/* those proc relopt with delayed_parsing = true should be put
			 * at the end of the relOpts array */
			continue;
		}
	...
	/* If we have delayed parsing options, it's time to add them. */
	if (j < num_options) {
		for (i = 0; procRelOpts[i].gen.name; ++i)
		{
			if (procRelOpts[i].delayed_parsing) {
...
add_local_reloption(local_relopts *relopts, relopt_gen *newoption, int offset)
{
	...
	if (opt->option->type == RELOPT_TYPE_PROC &&
		((relopt_proc *) opt->option)->delayed_parsing) {
		/* delayed parsing items are put at the end */
		relopts->options = lappend(relopts->options, opt);
	} else {
		relopts->options = lcons(opt, relopts->options);
	}
}
				   ...
				   LOCKMODE lockmode)
{
	...
	switch (type)
	{
		...
		case RELOPT_TYPE_STRING:
			...
			break;
		case RELOPT_TYPE_TYP:
			size = sizeof(relopt_typ);
			break;
		case RELOPT_TYPE_PROC:
			size = sizeof(relopt_proc);
			break;
		default:
...
/*
 * init_typ_reloption
 *		Allocate and initialize a new type reloption
 */
static relopt_typ *
init_typ_reloption(bits32 kinds, const char *name, const char *desc,
				   Oid default_val, int32 default_typemod,
				   LOCKMODE lockmode)
{
	relopt_typ *newoption;

	newoption = (relopt_typ *) allocate_reloption(kinds, RELOPT_TYPE_TYP,
												  name, desc, lockmode);
	newoption->default_val = default_val;
	newoption->default_typemod = default_typemod;
...
/*
 * add_typ_reloption
 *		Add a new type reloption
 */
void
add_typ_reloption(bits32 kinds, const char *name, const char *desc,
				  Oid default_val, int32 default_typemod,
				  LOCKMODE lockmode)
{
	relopt_typ *newoption = init_typ_reloption(kinds, name, desc,
											   default_val, default_typemod,
											   lockmode);
...
/*
 * add_local_typ_reloption
 *		Add a new local type reloption
 *
 * 'offset' is offset of an Oid-typed field followed by an int32-typed field.
 */
void
add_local_typ_reloption(local_relopts *relopts, const char *name,
						const char *desc, Oid default_val,
						int32 default_typemod, int offset)
{
	relopt_typ *newoption = init_typ_reloption(RELOPT_KIND_LOCAL,
											   name, desc, default_val,
											   default_typemod, 0);
...
/*
 * init_proc_reloption
 *		Allocate and initialize a new proc reloption
 */
static relopt_proc *
init_proc_reloption(bits32 kinds, const char *name, const char *desc,
					Oid default_val, bool delayed_parsing,
					int nargs,
					Oid *argtypes,
					get_arg_info_for_relopt_proc get_arg_info_cb,
					LOCKMODE lockmode)
{
	relopt_proc *newoption;

	newoption = (relopt_proc *) allocate_reloption(kinds, RELOPT_TYPE_PROC,
												   name, desc, lockmode);
	newoption->default_val = default_val;
	newoption->delayed_parsing = delayed_parsing;
	newoption->nargs = nargs;
	if (nargs > 0) {
		int i;
		for (i = 0; i < nargs; ++i) {
			newoption->argtypes[i] = argtypes[i];
		}
	}
	newoption->get_arg_info_cb = get_arg_info_cb;
...
/*
 * add_proc_reloption
 *		Add a new proc reloption
 */
void
add_proc_reloption(bits32 kinds, const char *name, const char *desc,
				   Oid default_val, bool delayed_parsing,
				   int nargs, Oid *argtype,
				   get_arg_info_for_relopt_proc get_arg_info_cb,
				   LOCKMODE lockmode)
{
	relopt_proc *newoption = init_proc_reloption(
								kinds, name, desc,
								default_val, delayed_parsing,
								nargs, argtype, get_arg_info_cb,
								lockmode);
...
/*
 * add_local_proc_reloption
 *		Add a new local proc reloption
 *
 * 'offset' is offset of an Oid-typed field
 */
void
add_local_proc_reloption(local_relopts *relopts, const char *name,
						 const char *desc, Oid default_val,
						 bool delayed_parsing, int nargs, Oid *argtypes,
						 get_arg_info_for_relopt_proc get_arg_info_cb,
						 int offset)
{
	relopt_proc *newoption = init_proc_reloption(
								RELOPT_KIND_LOCAL,
								name, desc, default_val, delayed_parsing,
								nargs, argtypes, get_arg_info_cb, 0);
	...
	if (numoptions > 0)
	{
		...
		for (i = 0, j = 0; relOpts[i]; i++)
		{
			if (relOpts[i]->kinds & kind)
			{
				...
				if (relOpts[i]->type == RELOPT_TYPE_PROC) {
					reloptions[j].values.proc_val.func_name_str = NULL;
				}
				j++;
			}
		}
...
parseLocalRelOptions(local_relopts *relopts, Datum options, bool validate)
{
	...
	foreach(lc, relopts->options)
	{
		...
		if (opt->option->type == RELOPT_TYPE_PROC) {
			values[i].values.proc_val.func_name_str = NULL;
		}
					...
					bool validate)
{
	...
	switch (option->gen->type)
	{
		...
		case RELOPT_TYPE_TYP:
			{
				parsed = parse_typ(value, &option->values.type_val.typeOid,
								   &option->values.type_val.typemod);
				if (validate && !parsed)
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
							 errmsg("invalid value for type option \"%s\": %s",
									option->gen->name, value)));
			}
			break;
		...
		case RELOPT_TYPE_PROC:
			{
				List *name;	
				relopt_proc *optproc = (relopt_proc *) option->gen;
				if (optproc->delayed_parsing) {
					/* will parse it when we fill the relopt_parse_elt */
					option->values.proc_val.func_name_str = value;
					parsed = false;
					nofree = true;
					break;
				}

				name = stringToQualifiedNameList(value);
				/* LookupFuncName will raise an error if the name is not found
				 * or ambiguous. */
				option->values.proc_val.procOid = LookupFuncName(
					name,
					optproc->nargs,
					optproc->argtypes,
					false);
...
static bool parse_typ(const char *value, Oid *typeOid, int32 *typemod) {
	...
	/* the 4th arg missing_ok = false so that parseTypeString()
	 * will elog(ERROR) when the type is not found. */
	parseTypeString(value, typeOid, typemod, false);
	return true;
}
					...
					case RELOPT_TYPE_TYP:
						{
							if (options[i].isset) {
								*(Oid *) itempos =
									options[i].values.type_val.typeOid;
								*(int32 *) (itempos + sizeof(Oid)) =
									options[i].values.type_val.typemod;
							} else {
								relopt_typ *opttyp =
									(relopt_typ *) options[i].gen;
								*(Oid *) itempos = opttyp->default_val;
								*(int32 *) (itempos + sizeof(Oid)) =
									opttyp->default_typemod;
							}
						}
						break;
					case RELOPT_TYPE_PROC:
						{
							if (options[i].isset) {
								*(Oid *) itempos =
									options[i].values.proc_val.procOid;
							} else {
								relopt_proc *procopt =
									(relopt_proc *) options[i].gen;
								if (options[i].values.proc_val.func_name_str) {
									/* this is delayed parsing */
									Oid parsing_result;
									int nargs;
									List *name;
									Oid *argtype = (Oid *) palloc(
										sizeof(Oid) * FUNC_MAX_ARGS);
									
									Assert(procopt->delayed_parsing);
									Assert(procopt->get_arg_info_cb);
									(procopt->get_arg_info_cb)(
										rdopts,
										&nargs,
										argtype);
									
									name = stringToQualifiedNameList(
										options[i].values.proc_val.func_name_str);
									parsing_result = LookupFuncName(
										name,
										nargs,
										argtype,
										false);
									*(Oid *) itempos = parsing_result;
									
									list_free(name);
									pfree(argtype);
									pfree(options[i].values.proc_val.func_name_str);
								} else {
									/* default value */
									*(Oid *) itempos =
										procopt->default_val;
