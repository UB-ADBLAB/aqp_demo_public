 ...
 *
 * abtvalidate.c
 *	  Opclass validator for aggregate btree.
 *
 *	  Copied and adapted from src/backend/access/abtree/abtvalidate.c.
 *
 * IDENTIFICATION
 *	  src/backend/access/abtree/abtvalidate.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
abtvalidate(Oid opclassoid)
{
	...
	for (i = 0; i < proclist->n_members; i++)
	{
		...
		switch (procform->amprocnum)
		{
			case ABTORDER_PROC:
				...
				break;
			case ABTSORTSUPPORT_PROC:
				ok = check_amproc_signature(procform->amproc, VOIDOID, true,
											1, 1, INTERNALOID);
				break;
			case ABTINRANGE_PROC:
				ok = check_amproc_signature(procform->amproc, BOOLOID, true,
											...
											BOOLOID, BOOLOID);
				break;
			case ABTEQUALIMAGE_PROC:
				ok = check_amproc_signature(procform->amproc, BOOLOID, true,
											1, 1, OIDOID);
				break;
			case ABTOPTIONS_PROC:
	...
	foreach(lc, grouplist)
	{
		...
		if (thisgroup->operatorset == 0 &&
			thisgroup->functionset == (1 << ABTINRANGE_PROC))
			continue;
		...
		if ((thisgroup->functionset & (1 << ABTORDER_PROC)) == 0)
		{
			ereport(INFO,
					(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
