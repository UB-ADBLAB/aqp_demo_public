 ...
 *
 * abtsplitloc.c
 *	  Choose split point code for aggregate btree.
 *
 *	  Copied and adapted from src/backend/access/nbtree/nbtsplitloc.c.
 *
 * IDENTIFICATION
 *	  src/backend/access/abtree/abtsplitloc.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
typedef enum
{
	...
	ABT_SPLIT_DEFAULT,				/* give some weight to truncation */
	ABT_SPLIT_MANY_DUPLICATES,		/* find minimally distinguishing point */
	ABT_SPLIT_SINGLE_VALUE			/* leave left page almost full */
} ABTFindSplitStrat;
...
typedef struct
{
	...
	/* split point identifying fields (returned by _abt_findsplitloc) */
...
typedef struct
{
	/* context data for _abt_recsplitloc */
	...
	bool		newitem_w_tid;	/* whether the new item has TID (pivot only) */
	...
	ABTSplitPoint *splits;			/* all candidate split points for page */
	int			interval;		/* current range of acceptable split points */
} ABTFindSplitData;

static void _abt_recsplitloc(ABTFindSplitData *state,
							OffsetNumber firstrightoff, bool newitemonleft,
							int olddataitemstoleft,
							Size firstrightofforigpagetuplesz,
							ABTCachedAggInfo agg_info);
static void _abt_deltasortsplits(ABTFindSplitData *state, double fillfactormult,
								bool usemult);
static int	_abt_splitcmp(const void *arg1, const void *arg2);
static bool _abt_afternewitemoff(ABTFindSplitData *state, OffsetNumber maxoff,
								int leaffillfactor, bool *usemult);
static bool _abt_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid);
static OffsetNumber _abt_bestsplitloc(ABTFindSplitData *state, int perfectpenalty,
									 bool *newitemonleft, ABTFindSplitStrat strategy);
static int	_abt_defaultinterval(ABTFindSplitData *state);
static int	_abt_strategy(ABTFindSplitData *state, ABTSplitPoint *leftpage,
						 ABTSplitPoint *rightpage, ABTFindSplitStrat *strategy);
static void _abt_interval_edges(ABTFindSplitData *state,
							   ABTSplitPoint **leftinterval, ABTSplitPoint **rightinterval);
static inline int _abt_split_penalty(ABTFindSplitData *state, ABTSplitPoint *split);
static inline IndexTuple _abt_split_lastleft(ABTFindSplitData *state,
											ABTSplitPoint *split);
static inline IndexTuple _abt_split_firstright(ABTFindSplitData *state,
											  ABTSplitPoint *split);


/*
 *	_abt_findsplitloc() -- find an appropriate place to split a page.
 *
 ...
 */
OffsetNumber
_abt_findsplitloc(Relation rel,
				 Page origpage,
				 ...
				 ABTCachedAggInfo agg_info)
{
	...
	int			leftspace,
				...
				perfectpenalty,
				leaffillfactor;
	ABTFindSplitData state;
	ABTFindSplitStrat strategy;
	ItemId		itemid;
	OffsetNumber offnum,
				maxoff,
				firstrightoff;
	...
	bool		usemult;
	ABTSplitPoint	leftpage,
				rightpage;

	opaque = (ABTPageOpaque) PageGetSpecialPointer(origpage);
	...
	leaffillfactor = ABTGetFillFactor(rel);
	...
	state.newitem_w_tid = !ABT_P_ISLEAF(opaque) &&
						  ABTPivotTupleHasHeapTID(newitem);
	state.is_leaf = ABT_P_ISLEAF(opaque);
	state.is_rightmost = ABT_P_RIGHTMOST(opaque);
	...
	Assert(!ABTreeTupleIsPosting(newitem));
	...
	state.splits = palloc(sizeof(ABTSplitPoint) * state.maxsplits);
	state.nsplits = 0;
	...
	for (offnum = ABT_P_FIRSTDATAKEY(opaque);
		 offnum <= maxoff;
		 offnum = OffsetNumberNext(offnum))
	{
		 ...
		 * page. (_abt_recsplitloc() will reject the split when there are no
		 * previous items, which we rely on.)
		 */
		if (offnum < newitemoff)
			_abt_recsplitloc(&state, offnum, false, olddataitemstoleft, itemsz,
							 agg_info);
		else if (offnum > newitemoff)
			_abt_recsplitloc(&state, offnum, true, olddataitemstoleft, itemsz,
							 agg_info);
		else
		{
			/*
	...
	if (newitemoff > maxoff)
		_abt_recsplitloc(&state, newitemoff, false, olddataitemstotal, 0,
						 agg_info);
	 ...
	 * considered.  See _abt_defaultinterval().
	 */
	if (!state.is_leaf)
	{
		/* fillfactormult only used on rightmost page */
		usemult = state.is_rightmost;
		fillfactormult = ABTREE_NONLEAF_FILLFACTOR / 100.0;
	}
	...
	else if (_abt_afternewitemoff(&state, maxoff, leaffillfactor, &usemult))
	{
		/*
		...
		else
		{
			...
			{
				ABTSplitPoint *split = state.splits + i;
	...
	_abt_deltasortsplits(&state, fillfactormult, usemult);

	/* Determine split interval for default strategy */
	state.interval = _abt_defaultinterval(&state);
	 ...
	 */
	perfectpenalty = _abt_strategy(&state, &leftpage, &rightpage, &strategy);

	if (strategy == ABT_SPLIT_DEFAULT)
	{
		/*
	 ...
	 */
	else if (strategy == ABT_SPLIT_MANY_DUPLICATES)
	{
	...
	else if (strategy == ABT_SPLIT_SINGLE_VALUE)
	{
		Assert(state.is_leaf);
		/* Split near the end of the page */
		usemult = true;
		fillfactormult = ABTREE_SINGLEVAL_FILLFACTOR / 100.0;
	 ...
	 */
	firstrightoff = _abt_bestsplitloc(&state, perfectpenalty, newitemonleft,
									 strategy);
	pfree(state.splits);
 ...
 */
static void
_abt_recsplitloc(ABTFindSplitData *state,
				...
				ABTCachedAggInfo agg_info)
{
	...
	Size		firstrightsz;
	/* 
	 * We intentionally misuse postingsz to denote any size that can be purged
	 * from the high key item, which could include some extra
	 */
	Size		postingsz = 0;
	...
	if (newitemisfirstright)
	{
		...
		if (state->is_leaf)
		{
			/* agg and xmin will be purged from high key anyway. */
			postingsz = agg_info->extra_space_for_nonpivot_tuple;
		}
		else
		{
			if (state->newitem_w_tid)
			{
				postingsz = agg_info->extra_space_when_tid_present -
					MAXALIGN(sizeof(ItemPointerData));
			}
			else
			{
				postingsz = agg_info->extra_space_when_tid_missing;
			}
		}
	}
	else
	{
		...
		if (state->is_leaf)
		{
			if (firstrightsz > 64)
			{
				...
				/* This truncates both agg and xmin arrays. */
				if (ABTreeTupleIsPosting(newhighkey))
					postingsz = IndexTupleSize(newhighkey) -
						ABTreeTupleGetPostingOffset(newhighkey);
			}
		}
		else
		{
			/* 
			 * Take the lower bound of the space to be purged without
			 * accessing the actual index tuple.
			 */
			
			postingsz = agg_info->extra_space_when_tid_present -
				MAXALIGN(sizeof(ItemPointerData));
			if (postingsz > agg_info->extra_space_when_tid_missing)
				postingsz = agg_info->extra_space_when_tid_missing;
		}
	}
	...
	if (state->is_leaf)
		...
		leftfree -= (int16) (firstrightsz - postingsz);
	 ...
	 * data from the first item that winds up on the right page. However,
	 * there's still the space for the aggregation value.
	 */
	if (!state->is_leaf)
		rightfree += (int16) firstrightsz -
			(int16) (MAXALIGN(sizeof(IndexTupleData)) +
 ...
 */
static void
_abt_deltasortsplits(ABTFindSplitData *state, double fillfactormult,
					bool usemult)
{
	for (int i = 0; i < state->nsplits; i++)
	{
	...
	qsort(state->splits, state->nsplits, sizeof(ABTSplitPoint), _abt_splitcmp);
}

/*
 * qsort-style comparator used by _abt_deltasortsplits()
 */
static int
_abt_splitcmp(const void *arg1, const void *arg2)
{
	ABTSplitPoint *split1 = (ABTSplitPoint *) arg1;
	ABTSplitPoint *split2 = (ABTSplitPoint *) arg2;
 ...
 */
static bool
_abt_afternewitemoff(ABTFindSplitData *state, OffsetNumber maxoff,
					int leaffillfactor, bool *usemult)
{
	int16		nkeyatts;
	ItemId		itemid;
	...
	if (state->newitemoff == ABTP_FIRSTKEY)
		return false;
	...
	if (state->newitemoff > maxoff)
	{
		...
		keepnatts = _abt_keep_natts_fast(state->rel, tup, state->newitem);
	...
	if (ABTreeTupleIsPosting(tup) ||
		!_abt_adjacenthtid(&tup->t_tid, &state->newitem->t_tid))
		return false;
 ...
 */
static bool
_abt_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid)
{
	BlockNumber lowblk,
				highblk;
 ...
 * splitting a leaf page or an internal page.  See _abt_split_penalty() for
 * details.
 *
 ...
 */
static OffsetNumber
_abt_bestsplitloc(ABTFindSplitData *state, int perfectpenalty,
				 bool *newitemonleft, ABTFindSplitStrat strategy)
{
	int			bestpenalty,
				lowsplit;
	...
	for (int i = lowsplit; i < highsplit; i++)
	{
		...
		penalty = _abt_split_penalty(state, state->splits + i);
	 ...
	 * leftmost page.
	 */
	if (strategy == ABT_SPLIT_MANY_DUPLICATES && !state->is_rightmost &&
...
#define ABT_LEAF_SPLIT_DISTANCE			0.050
#define ABT_INTERNAL_SPLIT_DISTANCE		0.075
 ...
 * interval is applied within _abt_bestsplitloc().
 *
 ...
 * which case the interval also changes -- see _abt_strategy()).
 *
 ...
 */
static int
_abt_defaultinterval(ABTFindSplitData *state)
{
	ABTSplitPoint *spaceoptimal;
	int16		tolerance,
				lowleftfree,
				lowrightfree,
				highleftfree,
	 ...
	 */
	if (state->is_leaf)
		tolerance = state->olddataitemstotal * ABT_LEAF_SPLIT_DISTANCE;
	else
		tolerance = state->olddataitemstotal * ABT_INTERNAL_SPLIT_DISTANCE;
 ...
 * passed to _abt_bestsplitloc() as a final constraint on how far caller is
 * willing to go to avoid appending a heap TID when using the many duplicates
 * strategy (it also saves _abt_bestsplitloc() useless cycles).
 */
static int
_abt_strategy(ABTFindSplitData *state, ABTSplitPoint *leftpage,
			 ABTSplitPoint *rightpage, ABTFindSplitStrat *strategy)
{
	IndexTuple	leftmost,
				rightmost;
	ABTSplitPoint *leftinterval,
			   *rightinterval;
	int			perfectpenalty;
	int			indnkeyatts = IndexRelationGetNumberOfKeyAttributes(state->rel);

	/* Assume that alternative strategy won't be used for now */
	*strategy = ABT_SPLIT_DEFAULT;
	 ...
	 */
	_abt_interval_edges(state, &leftinterval, &rightinterval);
	leftmost = _abt_split_lastleft(state, leftinterval);
	rightmost = _abt_split_firstright(state, rightinterval);
	 ...
	 */
	perfectpenalty = _abt_keep_natts_fast(state->rel, leftmost, rightmost);
	 ...
	 */
	leftmost = _abt_split_lastleft(state, leftpage);
	rightmost = _abt_split_firstright(state, rightpage);
	...
	if (perfectpenalty <= indnkeyatts)
	{
		*strategy = ABT_SPLIT_MANY_DUPLICATES;
		 ...
		 * before a final decision is made, within _abt_bestsplitloc().
		 */
		return indnkeyatts;
	}
	 ...
	 */
		...
		*strategy = ABT_SPLIT_SINGLE_VALUE;
	else
	{
		ItemId		itemid;
		IndexTuple	hikey;

		itemid = PageGetItemId(state->origpage, ABTP_HIKEY);
		hikey = (IndexTuple) PageGetItem(state->origpage, itemid);
		perfectpenalty = _abt_keep_natts_fast(state->rel, hikey,
											 state->newitem);
		if (perfectpenalty <= indnkeyatts)
			*strategy = ABT_SPLIT_SINGLE_VALUE;
		else
 ...
 */
static void
_abt_interval_edges(ABTFindSplitData *state, ABTSplitPoint **leftinterval,
				   ABTSplitPoint **rightinterval)
{
	int			highsplit = Min(state->interval, state->nsplits);
	ABTSplitPoint *deltaoptimal;
	...
	*rightinterval = NULL;
	...
	{
		ABTSplitPoint *distant = state->splits + i;
 ...
 */
static inline int
_abt_split_penalty(ABTFindSplitData *state, ABTSplitPoint *split)
{
	IndexTuple	lastleft;
	...
	lastleft = _abt_split_lastleft(state, split);
	firstright = _abt_split_firstright(state, split);

	return _abt_keep_natts_fast(state->rel, lastleft, firstright);
}
...
_abt_split_lastleft(ABTFindSplitData *state, ABTSplitPoint *split)
{
	ItemId		itemid;
 ...
 */
static inline IndexTuple
_abt_split_firstright(ABTFindSplitData *state, ABTSplitPoint *split)
{
	ItemId		itemid;
