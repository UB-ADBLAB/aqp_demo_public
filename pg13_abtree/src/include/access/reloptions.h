...
typedef enum relopt_kind
{
	...
	RELOPT_KIND_ABTREE = (1 << 12),
	/* if you add a new kind, make sure you update "last_default" too */
	RELOPT_KIND_LAST_DEFAULT = RELOPT_KIND_ABTREE,
...
typedef struct relopt_value
{
	...
	union
	{
		...
		struct {
			Oid		procOid;
			/* func_name_str == NULL when isset == NULL means
			 * there's no user specified value for this option. 
			 * func_name_str will point to a temporary buffer
			 * containing the function name if it is set and needs
			 * a delayed parsing.
			 * */
			char	*func_name_str;
		}			proc_val;
...
typedef struct relopt_typ
{
	relopt_gen				gen;
	Oid						default_val;
	int32					default_typemod;
} relopt_typ;

/* The argtypes argument will be an allocated array with space for at least
 * FUNC_MAX_ARGS Oid. */
typedef void (*get_arg_info_for_relopt_proc)(
		void *options,
		int *nargs,
		Oid argtypes[FUNC_MAX_ARGS]);
typedef struct relopt_proc
{
	relopt_gen				gen;
	Oid						default_val;
	bool					delayed_parsing;
	int						nargs;
	Oid						argtypes[FUNC_MAX_ARGS];
	get_arg_info_for_relopt_proc
							get_arg_info_cb;
} relopt_proc;
...
extern void add_string_reloption(bits32 kinds, const char *name, const char *desc,
								 ...
								 LOCKMODE lockmode);
extern void add_typ_reloption(bits32 kinds, const char *name, const char *desc,
							   Oid default_val, int32 default_typemod,
							   LOCKMODE lockmode);
extern void add_proc_reloption(bits32 kinds, const char *name, const char *desc,
								Oid default_val,
								bool delayed_parsing,
								int nargs,
								Oid *argtypes,
...
extern void add_local_typ_reloption(local_relopts *relopts,
									 const char *name, const char *desc,
									 Oid default_val, int32 default_typemod,
									 int offset);
extern void add_local_proc_reloption(
