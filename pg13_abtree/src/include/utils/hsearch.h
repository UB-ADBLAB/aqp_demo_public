...
typedef struct
{
	...
	bool		allow_delete;	/* whether to allow delete the previously
								   returned item */
	HASHELEMENT *curEntry;		/* current entry in bucket */
	HASHELEMENT	**prevEntryPtr;	/* the pointer to the previously returned */
} HASH_SEQ_STATUS;
...
#define hash_seq_init(status, hashp) \
	hash_seq_init_extended(status, hashp, false)
extern void hash_seq_init_extended(HASH_SEQ_STATUS *status, HTAB *hashp,
								   bool allow_delete);
#define hash_seq_search(status) \
	hash_seq_search_extended(status, false)
extern void *hash_seq_search_extended(HASH_SEQ_STATUS *status,
									  bool delete_prev);
