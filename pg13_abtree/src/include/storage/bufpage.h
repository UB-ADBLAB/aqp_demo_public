...
#ifndef FRONTEND
/* port/atomics.h may not be included from frontend code */
...
/* 
 * The following atomic LSN operations on page LSN are only available in
 * backend code.
 */

#define PagetAtomicGetLSN(page) \
	PageXLogRecPtrAtomicGet(&((PageHeader) (page))->pd_lsn)
#define PageCompareExchangeLSN(page, old_lsn_ptr, new_lsn) \
	PageXLogRecPtrCompareExchange(&((PageHeader) (page))->pd_lsn, old_lsn_ptr, \
								  new_lsn)
static inline uint64
PageXLogRecPtrAtomicGet(void *recptr)
{
	uint64 val = pg_atomic_read_u64((volatile pg_atomic_uint64 *) recptr);
	PageXLogRecPtr *ptr = (PageXLogRecPtr*) &val;
	return PageXLogRecPtrGet(*ptr);
}

/*
 * This is used currently by ABTree to concurrently install LSN into a page.
 * Only set the LSN if it's larger than the old LSN on the page.
 */
static inline void
PageAtomicSetLSN(Page page, uint64 new_lsn)
{
	pg_atomic_uint64	*rec_ptr =
		(pg_atomic_uint64 *) &((PageHeader) page)->pd_lsn;
	uint64				old_recptr;
	PageXLogRecPtr		new_recptr;
	
	PageXLogRecPtrSet(new_recptr, new_lsn);
	old_recptr = pg_atomic_read_u64(rec_ptr);
	do {
		if (PageXLogRecPtrGet(*(PageXLogRecPtr *) &old_recptr) > new_lsn) break;
	} while (!pg_atomic_compare_exchange_u64(rec_ptr, &old_recptr,
											 *(uint64 *) &new_recptr));
}
